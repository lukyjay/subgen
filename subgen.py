...
# Priority queue to hold tasks with different priorities
...
def form_get():
    ...
    for var_name, var_info in env_variables.items():
        value = os.getenv(var_name, env_values.get(var_name, var_info['default'])) if not isinstance(
            var_info['default'], bool) else convert_to_bool(
            os.getenv(var_name, env_values.get(var_name, var_info['default'])))
...
async def form_post(request: Request):
    ...
    with open(env_path, "w") as file:
        for key, value in form_data.items():
            ...
            value = value.strip() if not isinstance(env_variables[key]["default"], bool) else convert_to_bool(
                value.strip())
...
def status():
    ...
    docker_status = "Docker" if in_docker else "Standalone"
    return {
        "version": f"Subgen {subgen_version}, stable-ts {stable_whisper.__version__}, whisper {whisper.__version__} ({docker_status})"}
...
def receive_tautulli_webhook(
    ...
    if source == "Tautulli":
        ...
        return {
            "This doesn't appear to be a properly configured Tautulli webhook, please review the instructions again!"}
...
def receive_jellyfin_webhook(
    ...
    if "Jellyfin-Server" in user_agent:
        ...
        return {
            "This doesn't appear to be a properly configured Jellyfin webhook, please review the instructions again!"}
...
async def asr(
        ...
        # not used by Bazarr/always False
    ...
    try:
        ...
        task_queue.put((2, future, audio_data, task, language, True ))
        result = await future
    except Exception as e:
...
def detect_language(
    ...
    try:
        ...
        return {"detected_language": whisper_languages.get(detected_lang_code, detected_lang_code),
                "language_code": detected_lang_code}

def new_gen_subtitles(audio, transcribe_or_translate: str, forceLanguage=None, bazarr=False, max_workers=3):
    ...
    with semaphore:
        ...
        try:
            bazarr = isinstance(audio, np.ndarray)
            start_model()
            start_time = time.time()
            ...
            if not bazarr:
                ...
                message = None
                if has_subtitle_language(audio, skipifinternalsublang):
                    message = f"{audio} already has an internal subtitle we want, skipping generation"
                elif os.path.exists(audio.rsplit('.', 1)[0] + subextension):
                    message = f"{audio} already has a subtitle created for this, skipping it"
                elif os.path.exists(audio.rsplit('.', 1)[0] + subextensionSDH):
                    message = f"{audio} already has a SDH subtitle created for this, skipping it"
                if message != None:
                    logging.info(message)
                    return message
            logging.info(f"Transcribing {'Bazarr/ASR' if isinstance(audio, np.ndarray) else os.path.basename(audio)}")

            if custom_regroup:
                result = executor.submit(model.transcribe_stable, audio, language=forceLanguage, task=transcribe_or_translate, progress_callback=progress, initial_prompt=custom_model_prompt, regroup=custom_regroup).result()
            else:
                result = executor.submit(model.transcribe_stable, audio, language=forceLanguage, task=transcribe_or_translate, progress_callback=progress, initial_prompt=custom_model_prompt).result()
            appendLine(result)
            elapsed_time = time.time() - start_time
            minutes, seconds = divmod(int(elapsed_time), 60)
            ...
            logging.info(f"Transcription of {'Bazarr/ASR' if isinstance(audio, np.ndarray) else os.path.basename(audio)} is completed, it took {minutes} minutes and {seconds} seconds to complete.")
        except Exception as e:
            logging.info(f"Error processing or transcribing {'Bazarr/ASR' if isinstance(audio, np.ndarray) else os.path.basename(audio)}: {e}")
        finally:
            if bazarr:
                return result
def worker():
    while True:
        priority, future, file, transcribe_or_translate, forceLanguage = task_queue.get()
        try:
            logging.info(f"There are {task_queue.qsize()} items in the queue")
            result = new_gen_subtitles(file, transcribe_or_translate, forceLanguage)
...
def start_model():
    ...
    if model is None:
        ...
        model = stable_whisper.load_faster_whisper(whisper_model, download_root=model_location,
                                                   device=transcribe_device, cpu_threads=whisper_threads,
                                                   num_workers=concurrent_transcriptions, compute_type=compute_type)
...
def has_subtitle_language(video_file, target_language):
    try:
        with av.open(video_file) as container:
            subtitle_stream = next((stream for stream in container.streams if
                                    stream.type == 'subtitle' and 'language' in stream.metadata and stream.metadata[
                                        'language'] == target_language), None)
...
def transcribe_existing(transcribe_folders, forceLanguage=None):
    ...
    for path in transcribe_folders:
        ...
        for root, dirs, files in os.walk(path):
            for file in files:
                ...
                if has_audio(file_path) and needsubs(path):
                    task_queue.put((2, None, path_mapping(file_path), transcribe_or_translate, forceLanguage))
    ...
    if os.path.isfile(path):
        if has_audio(path) and needsubs(path):
            #gen_subtitles(path_mapping(path), transcribe_or_translate, False, forceLanguage)
            task_queue.put((2, None, path_mapping(path), transcribe_or_translate, forceLanguage))
...
env_variables = {
    "TRANSCRIBE_DEVICE": {
        "description": "Can transcribe via gpu (Cuda only) or cpu. Takes option of 'cpu', 'gpu', 'cuda'.",
        "default": "cpu", "value": ""},
    "WHISPER_MODEL": {
        "description": "Can be: 'tiny', 'tiny.en', 'base', 'base.en', 'small', 'small.en', 'medium', 'medium.en', 'large-v1','large-v2', 'large-v3', 'large', 'distil-large-v2', 'distil-medium.en', 'distil-small.en'",
        "default": "medium", "value": ""},
    "CONCURRENT_TRANSCRIPTIONS": {"description": "Number of files it will transcribe in parallel", "default": "2",
                                  "value": ""},
    "WHISPER_THREADS": {"description": "Number of threads to use during computation", "default": "4", "value": ""},
    "MODEL_PATH": {
        "description": "This is where the WHISPER_MODEL will be stored. This defaults to placing it where you execute the script in the folder 'models'",
        "default": "./models", "value": ""},
    "PROCADDEDMEDIA": {
        "description": "Will gen subtitles for all media added regardless of existing external/embedded subtitles (based off of SKIPIFINTERNALSUBLANG)",
        "default": True, "value": ""},
    "PROCMEDIAONPLAY": {
        "description": "Will gen subtitles for all played media regardless of existing external/embedded subtitles (based off of SKIPIFINTERNALSUBLANG)",
        "default": True, "value": ""},
    "NAMESUBLANG": {
        "description": "Allows you to pick what it will name the subtitle. Instead of using EN, I'm using AA, so it doesn't mix with existing external EN subs, and AA will populate higher on the list in Plex.",
        "default": "aa", "value": ""},
    "SKIPIFINTERNALSUBLANG": {
        "description": "Will not generate a subtitle if the file has an internal sub matching the 3 letter code of this variable",
        "default": "eng", "value": ""},
    "WORD_LEVEL_HIGHLIGHT": {"description": "Highlights each word as it's spoken in the subtitle.", "default": False,
                             "value": ""},
    "PLEXSERVER": {"description": "This needs to be set to your local plex server address/port",
                   "default": "http://plex:32400", "value": ""},
    "PLEXTOKEN": {"description": "This needs to be set to your plex token", "default": "token here", "value": ""},
    "JELLYFINSERVER": {"description": "Set to your Jellyfin server address/port", "default": "http://jellyfin:8096",
                       "value": ""},
    "JELLYFINTOKEN": {"description": "Generate a token inside the Jellyfin interface", "default": "token here",
                      "value": ""},
    "WEBHOOKPORT": {"description": "Change this if you need a different port for your webhook", "default": "9000",
                    "value": ""},
    "USE_PATH_MAPPING": {
        "description": "Similar to sonarr and radarr path mapping, this will attempt to replace paths on file systems that don't have identical paths. Currently only support for one path replacement.",
        "default": False, "value": ""},
    "PATH_MAPPING_FROM": {"description": "This is the path of my media relative to my Plex server", "default": "/tv",
                          "value": ""},
    "PATH_MAPPING_TO": {
        "description": "This is the path of that same folder relative to my Mac Mini that will run the script",
        "default": "/Volumes/TV", "value": ""},
    "TRANSCRIBE_FOLDERS": {
        "description": "Takes a pipe '|' separated list and iterates through and adds those files to be queued for subtitle generation if they don't have internal subtitles",
        "default": "", "value": ""},
    "TRANSCRIBE_OR_TRANSLATE": {
        "description": "Takes either 'transcribe' or 'translate'. Transcribe will transcribe the audio in the same language as the input. Translate will transcribe and translate into English.",
        "default": "transcribe", "value": ""},
    "COMPUTE_TYPE": {
        "description": "Set compute-type using the following information: https://github.com/OpenNMT/CTranslate2/blob/master/docs/quantization.md",
        "default": "auto", "value": ""},
    "DEBUG": {
        "description": "Provides some debug data that can be helpful to troubleshoot path mapping and other issues. If set to true, any modifications to the script will auto-reload it (if it isn't actively transcoding). Useful to make small tweaks without re-downloading the whole file.",
        "default": True, "value": ""},
    "FORCE_DETECTED_LANGUAGE_TO": {
        "description": "This is to force the model to a language instead of the detected one, takes a 2 letter language code.",
        "default": "", "value": ""},
    "CLEAR_VRAM_ON_COMPLETE": {
        "description": "This will delete the model and do garbage collection when queue is empty. Good if you need to use the VRAM for something else.",
        "default": True, "value": ""},
    "UPDATE": {
        "description": "Will pull latest subgen.py from the repository if True. False will use the original subgen.py built into the Docker image. Standalone users can use this with launcher.py to get updates.",
        "default": False, "value": ""},
    "APPEND": {
        "description": "Will add the following at the end of a subtitle: 'Transcribed by whisperAI with faster-whisper ({whisper_model}) on {datetime.now()}'",
        "default": False, "value": ""},
    "MONITOR": {
        "description": "Will monitor TRANSCRIBE_FOLDERS for real-time changes to see if we need to generate subtitles",
        "default": False, "value": ""},
    "USE_MODEL_PROMPT": {
        "description": "When set to True, will use the default prompt stored in greetings_translations 'Hello, welcome to my lecture.' to try and force the use of punctuation in transcriptions that don't.",
        "default": False, "value": ""},
    "CUSTOM_MODEL_PROMPT": {
        "description": "If USE_MODEL_PROMPT is True, you can override the default prompt (See: [prompt engineering in whisper](https://medium.com/axinc-ai/prompt-engineering-in-whisper-6bb18003562d%29) for great examples).",
        "default": "", "value": ""},
    "LRC_FOR_AUDIO_FILES": {
        "description": "Will generate LRC (instead of SRT) files for filetypes: '.mp3', '.flac', '.wav', '.alac', '.ape', '.ogg', '.wma', '.m4a', '.m4b', '.aac', '.aiff'",
        "default": True, "value": ""},
    "CUSTOM_REGROUP": {
        "description": "Attempts to regroup some of the segments to make a cleaner looking subtitle. See #68 for discussion. Set to blank if you want to use Stable-TS default regroups algorithm of cm_sp=,* /，_sg=.5_mg=.3+3_sp=.* /。/?/？",
        "default": "cm_sl=84_sl=42++++++1", "value": ""}
}
